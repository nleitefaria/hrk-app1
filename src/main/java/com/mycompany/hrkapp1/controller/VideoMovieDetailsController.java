...
public class VideoMovieDetailsController 
{
	...
	@ApiOperation(value = "Get all movies details",response = Iterable.class)
    @ApiResponses(value = {
	...
	@ApiOperation(value = "Get movie details by id",response = Iterable.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successfully retrieved list"),
            @ApiResponse(code = 401, message = "You are not authorized to view the resource"),
            @ApiResponse(code = 403, message = "Accessing the resource you were trying to reach is forbidden"),
	...
	@GetMapping("/video/movies-details/{id}")
    public Mono<ResponseEntity<VideoMovieDetails>> findOne(@PathVariable(value = "id") String id) 
	{
        return repository.findById(new ObjectId(id))
                .map(videoMovieDetails -> ResponseEntity.ok(videoMovieDetails))
